...
Reed loadReed()
{
  ...
  Reed r=new Reed(float(parts[0]), float(parts[1]), int(parts[2]), float(parts[3]), float(parts[4]), float(parts[5]), float(parts[6]));
...
void saveReed()
{
  if (millis()-lastSaved>500)
  {
    ...
    String line=reed.reedWidth+ "," +reed.reedHeight+ "," +reed.slotNumber+ "," +reed.slotWidth+ "," +reed.slotHeight+ "," +reed.slotSpacing+","+reed.heddleWidth;
    println(line);
    output.println(line);
    output.flush();
    output.close();
